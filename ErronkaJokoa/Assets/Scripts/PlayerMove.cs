...
public class PlayerMove : MonoBehaviour
{
    ...
    {
        if(rot == 1 && horizontalInput < 0)//Eskumara begira baldin badago eta ezkerrera mugitzen bada jokalaria ezkerrera rotatzen da
        {
            transform.Rotate(0, 180, 0);
            rot = 0;
        }else if(rot == 0 && horizontalInput > 0)//Ezkerrera begira baldin badago eta eskumara mugitzen bada jokalaria eskumara rotatzen da
        {
            transform.Rotate(0, 180, 0);
            rot = 1;
        }
    ...
    {
        if((Input.GetMouseButtonDown(0) || Input.GetMouseButton(0)) && proximoDisparo <= Time.time){
